...
/**
 * Error event - error from LLM or execution.
 *
 * `severity` lets the UI distinguish real failures from soft/recoverable
 * situations so it doesn't scare users with a red "crashed" look when the
 * agent actually recovers or the user just paused on purpose:
 * - `fatal` (default when omitted, for backward compatibility): the task
 *   ended in failure. Should be rendered as an error (e.g. red).
 * - `warning`: a transient issue the agent is auto-recovering from (e.g. a
 *   malformed LLM response being retried). The task keeps going. Should be
 *   rendered as a soft warning (e.g. amber), not a hard error.
 * - `info`: an expected, user-initiated interruption (e.g. the user clicked
 *   "stop"). Not a failure at all. Should be rendered neutrally.
 */
...
export type AgentActivity =
	...
	| { type: 'error'; message: string; severity?: 'fatal' | 'warning' | 'info' }
