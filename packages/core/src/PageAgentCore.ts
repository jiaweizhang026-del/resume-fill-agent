...
export class PageAgentCore extends EventTarget {
	...
	constructor(config: PageAgentCoreConfig) {
		...
		this.#llm.addEventListener('retry', (e) => {
			...
			this.history.push({
				type: 'error',
				// The agent is auto-recovering by retrying, not failing the task.
				// Mark as 'warning' so the UI doesn't render this as a hard failure.
				severity: 'warning',
		...
		try {
			...
			while (true) {
				...
				try {
					...
					// A user-initiated stop is an expected interruption, not a crash:
					// use a distinct message + 'info' severity so the UI renders it
					// neutrally instead of as a red error.
					const message = isAbortError ? 'Task stopped' : String(error)
					const severity = isAbortError ? 'info' : 'fatal'
					this.#emitActivity({ type: 'error', message, severity })
					this.#emitHistoryChange({ type: 'error', message, severity, rawResponse: error })
