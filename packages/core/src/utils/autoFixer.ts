...
export function normalizeResponse(response: any, tools?: Map<string, PageAgentTool>): any {
	...
	// Guard against non-object results (e.g. the model returned a
	// Python-dict-style blob with single quotes, which JSON.parse rejects,
	// so `safeJsonParse` gave up and returned the raw string as-is).
	// Without this check, the assignments below would try to create a
	// property on a string primitive and crash with a low-level, unreadable
	// `TypeError: Cannot create property ... on string '...'` instead of a
	// clear, retryable error that tells the model what went wrong.
	if (typeof resolvedArguments !== 'object' || resolvedArguments === null) {
		throw new InvokeError(
			InvokeErrorTypes.INVALID_TOOL_ARGS,
			'Response is not valid JSON (e.g. it used single quotes instead of double quotes). ' +
				'You MUST return valid JSON with double-quoted keys and string values.'
		)
	}
