...
export class OpenAIClient implements LLMClient {
	...
	async invoke(
		...
		if (!response.ok) {
			...
			// Gateways aren't all OpenAI-shaped: some (e.g. our own resume-api proxy)
			// return `{ error: "plain string" }` instead of `{ error: { message: "..." } }`.
			// Falling through every known shape avoids surfacing an empty/misleading
			// message (previously this silently dropped the real reason and left
			// users staring at a bare "Authentication failed" with no explanation).
			const errorMessage =
			...
			if (response.status === 402) {
				throw new InvokeError(
					InvokeErrorTypes.INSUFFICIENT_CREDIT,
					`Insufficient credit: ${errorMessage}`,
					errorData
				)
			}
