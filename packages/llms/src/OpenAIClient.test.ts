...
describe('OpenAIClient.invoke — HTTP errors', () => {
	...
	it('maps status 402 to INSUFFICIENT_CREDIT', async () => {
		const { client, fetchMock } = makeClient()
		fetchMock.mockResolvedValue(jsonResponse({ error: 'out of credit' }, 402))
		await expect(client.invoke([], tools, signal)).rejects.toMatchObject({
			name: 'InvokeError',
			type: InvokeErrorTypes.INSUFFICIENT_CREDIT,
			message: expect.stringContaining('out of credit'),
		})
	})

	it('extracts the message when the gateway returns `error` as a plain string (not OpenAI-shaped)', async () => {
		const { client, fetchMock } = makeClient()
		fetchMock.mockResolvedValue(jsonResponse({ error: 'This product key has been disabled.' }, 403))
		await expect(client.invoke([], tools, signal)).rejects.toMatchObject({
			type: InvokeErrorTypes.AUTH_ERROR,
			message: expect.stringContaining('This product key has been disabled.'),
		})
	})
})
