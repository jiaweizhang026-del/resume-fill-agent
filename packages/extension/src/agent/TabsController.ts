...
export class TabsController {
	...
	async init(task: string, options: TabsInitOptions = {}) {
		...
		if (experimentalIncludeAllTabs) {
			...
			for (const tab of allTabs.tabs as chrome.tabs.Tab[]) {
				const isInitial = tab.id === this.initialTabId
				// The user's actively focused tab is always eligible, even if pinned.
				// Pinned status only excludes *other* tabs from being auto-picked up,
				// to avoid hijacking utility tabs (e.g. mail, chat) the user didn't ask about.
				if (tab.id && isContentScriptAllowed(tab.url) && (!tab.pinned || isInitial)) {
					this.addTab({
						id: tab.id,
						isInitial,
						url: tab.url,
			...
			// Pinned is intentionally NOT excluded here: this is the tab the user is
			// actually looking at when they send the task, so it must be usable.
		...
		// Intentionally NOT throwing when currentTabId ends up null (e.g. the
		// active tab is a restricted browser page like chrome://). Throwing
		// here would abort the task before the agent loop even starts, which
		// feels like the extension crashed rather than an agent replying.
		// Instead we let the loop run: `getBrowserState`/`remoteCallDomAction`
		// already surface a clear "no target page" message to the model, and
		// the anti-hallucination instructions on those + `open_new_tab` make
		// it respond via `done(success=false, ...)` — a normal chat turn that
		// asks the user to open the target page, instead of a hard error.
	}
	...
	async syncTabs(): Promise<void> {
		...
		// Recovery: when all tracked tabs are gone (e.g. user closed the
		// original tab and the agent has no fallback), auto-pick up the
		// window's active tab so the agent can keep working on whatever page
		// the user is looking at.
		if (this.currentTabId == null && !this.disposed) {
			const activeTabResult = await sendMessage({
				type: 'TAB_CONTROL',
				action: 'get_active_tab',
				payload: { windowId: this.windowId },
			})
			const activeTab = activeTabResult?.tab as chrome.tabs.Tab | undefined
			// Pinned is not excluded here either: this recovery path follows whatever
			// page the user is actively looking at once all tracked tabs are gone.
			if (activeTab?.id && isContentScriptAllowed(activeTab.url)) {
				debug('syncTabs: recovered active tab', activeTab.id, activeTab.url)
				this.addTab({
					id: activeTab.id,
					isInitial: false,
					url: activeTab.url,
				...
				await this.updateCurrentTabId(activeTab.id)
			}
		}
	}
