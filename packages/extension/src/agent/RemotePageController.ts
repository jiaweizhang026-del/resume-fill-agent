...
export class RemotePageController {
	...
	async getBrowserState(): Promise<BrowserState> {
		...
		if (!this.currentTabId || !isContentScriptAllowed(currentUrl)) {
			const errorContent = !this.currentTabId
				? '(no target page available — the tab this agent was working on was closed and no fallback tab was found. do not guess or open a random URL. call done with success=false and ask the user to open the target page.)'
				: '(this page cannot be automated — it is a restricted browser page. do not guess or open a random URL. call done with success=false and ask the user to open the target page.)'
			browserState = {
				url: currentUrl,
				title: currentTitle,
				header: '',
	...
	private async remoteCallDomAction(action: string, payload: any[]): Promise<DomActionReturn> {
		...
		if (!isContentScriptAllowed(await this.getCurrentUrl())) {
			return {
				success: false,
				message:
					'Operation not allowed on this page (restricted browser page). Do NOT guess or invent a URL. If you already know the exact target URL (e.g. given by the user), use open_new_tab to navigate there. Otherwise, call done with success=false and ask the user to open the target page manually.',
			}
		}
