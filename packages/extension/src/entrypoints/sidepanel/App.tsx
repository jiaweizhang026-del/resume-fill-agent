		...
		<div className="relative flex flex-col h-screen bg-background">
			...
			<header className="flex items-center justify-between border-b px-3 py-2">
				<div className="flex items-center gap-2">
					...
					<span className="text-sm font-medium">履历助手</span>
				</div>
			...
			<footer className="bg-background px-3 pb-3 pt-2">
				<InputGroup className="relative h-14 rounded-full bg-background px-1.5 shadow-[0_8px_24px_oklch(0.145_0_0_/_0.06)]">
					<InputGroupTextarea
						ref={textareaRef}
						placeholder="例如：根据我的履历填写当前页面"
						value={inputValue}
						onChange={(e) => setInputValue(e.target.value)}
						onKeyDown={handleKeyDown}
						disabled={isRunning}
						className="h-14 min-h-14 max-h-14 overflow-y-auto py-4 pr-14 text-sm leading-5"
					/>
					<InputGroupAddon align="inline-end" className="absolute bottom-0 right-1.5 top-0 py-0">
						{isRunning ? (
							<InputGroupButton
								size="icon-sm"
								variant="destructive"
							...
							<InputGroupButton
								...
								className="size-9 cursor-pointer rounded-full transition-[background-color,color,transform] disabled:bg-muted disabled:text-muted-foreground/35 disabled:opacity-100 disabled:shadow-none"
								aria-label="Send"
								title="Send"
							>
