...
// Friendly card for a specific, well-understood failure: the product key
// ran out of usage credit. Shown instead of the raw technical error message
// (e.g. "InvokeError: Authentication failed: ...") because that text gives
// users no idea what happened or what to do next - they just see the agent
// silently stop making progress.
function InsufficientCreditCard() {
	return (
		<div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-2.5">
			<div className="flex items-start gap-1.5">
				<CreditCard className="size-3 text-amber-500 shrink-0 mt-0.5" />
				<span className="text-xs text-amber-600 dark:text-amber-400">
					使用额度已用完，请充值后再试。
				</span>
			</div>
...
// Error card - styling depends on severity so users aren't scared by things
// that aren't real failures:
// - 'fatal' (default, e.g. task actually failed): red, looks like an error.
// - 'warning' (e.g. a malformed LLM response being auto-retried): amber, task keeps going.
// - 'info' (e.g. user clicked "stop"): neutral gray, not an error at all.
function ErrorCard({ event }: { event: AgentErrorEvent }) {
	const severity = event.severity ?? 'fatal'

	// The product key ran out of usage credit. Prefer this specific,
	// actionable message over the generic (and previously misleading, since
	// the underlying gateway error message often failed to parse) auth-error
	// text, regardless of severity.
	if (
		event.rawResponse instanceof InvokeError &&
		event.rawResponse.type === InvokeErrorTypes.INSUFFICIENT_CREDIT
	) {
		return <InsufficientCreditCard />
	}

	if (severity === 'info') {
		return (
			<div className="rounded-lg border border-muted-foreground/20 bg-muted/40 p-2.5">
				<div className="flex items-start gap-1.5">
					<XCircle className="size-3 text-muted-foreground shrink-0 mt-0.5" />
	...
	if (severity === 'warning') {
		return (
			<div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-2.5">
				<div className="flex items-start gap-1.5">
					<RefreshCw className="size-3 text-amber-500 shrink-0 mt-0.5" />
					<span className="text-xs text-amber-600 dark:text-amber-400">{event.message}</span>
				</div>
				<RawSection rawResponse={event.rawResponse} />
			</div>
		)
...
export function ActivityCard({ activity }: { activity: AgentActivity }) {
	const getActivityInfo = () => {
		switch (activity.type) {
			...
			case 'error': {
				// Match ErrorCard's severity-based coloring so the transient
				// activity indicator doesn't flash red for non-fatal cases
				// (e.g. user clicked "stop").
				const severity = activity.severity ?? 'fatal'
				const color =
					severity === 'info'
						...
						: severity === 'warning'
							? 'text-amber-500'
							: 'text-destructive'
				return { text: activity.message, color }
			}
		}
	}

	const info = getActivityInfo()
	const errorSeverity = activity.type === 'error' ? activity.severity ?? 'fatal' : null
	const dotColor =
	...
	return (
		<div className="flex items-center gap-2 rounded-lg border bg-muted/40 p-2.5 animate-pulse">
			<div className="relative">
				...
				<span
					className={cn('absolute -top-0.5 -right-0.5 size-1.5 rounded-full animate-ping', dotColor)}
				/>
			</div>
